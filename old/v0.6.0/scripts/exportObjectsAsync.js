/* globals getValue, getNotesRaw, nameImage, saveMedia, Jukebox, saveToFile, saveToArray, saveToCampaign, cleanupName, getBlob, timeout */
...
const exportCharacters = (async () => { // eslint-disable-line no-unused-vars
	...
	const exportRollTable = async (table, i) => {
		...
		await Promise.all(table.tableitems.models.map(async (item) => {
			...
			if (imgSrc && (exportImages || localImages)) { // MOVE TO HELPERS FFS
				let newImgName = await nameImage(imgSrc, itemData.name||'rti', filenameLog)
				if (newImgName) {
					if (exportImages && newImgName.search(/^!DUPE!/) === -1) imageQueue.push(`${imgSrc}|||${newImgName}`);
					else newImgName = newImgName.replace(/^!DUPE!/,'');
					itemData.image = newImgName || "";
				} else console.log(`Error in nameImage function ???`);
			}
	...
	const exportPage = async (page, i) => { // process & export a map page
		...
		await Promise.all(page.thegraphics.models.map(async (tok) => {
				...
				if (exportImages || localImages) { // process Images for download if requested
					let newImgName = await nameImage(imgSrc, imgName, filenameLog)
					if (newImgName) {
						if (exportImages && newImgName.search(/^!DUPE!/) === -1) imageQueue.push(`${imgSrc}|||${newImgName}`);
						else newImgName = newImgName.replace(/^!DUPE!/,'');
						tokData.image = newImgName || "";
					} else console.log(`Error in nameImage function ???`);
				}
	...
	const exportHandout = async (handout, i) => { //{same as Character sheet, with one Promise/blob getter for "notes"}
		...
		await timeout(1);
			...
			if ((exportImages || localImages) && handout.attributes.avatar) { // process Images for download if requested
				let imgSrc = handout.attributes.avatar;
				let newImgName = await nameImage(imgSrc, handout.attributes.name, filenameLog)
				if (newImgName) {
					if (exportImages && newImgName.search(/^!DUPE!/) === -1) imageQueue.push(`${imgSrc}|||${newImgName}`);
					else newImgName = newImgName.replace(/^!DUPE!/,'');
	...
	const prepareCampaign = async () => {
		...
		Promise.all([pageDone, charDone, handDone, tableDone, macroDone]).then(async () => {
