...
const saveMedia = (mediaType, inputArray=[], campaignName) => {
	...
	let suffix = ['original', 'max', 'med', 'thumb'];
	let failedImages = [];
			...
			//.then(async (resp) => {
				if (mediaType === 'Images' && (!resp || resp?.status !== 200)) {
					...
					for (imgTry=0; imgTry<4; imgTry++) {
						...
						let newResp = await fetch(newUrl);
